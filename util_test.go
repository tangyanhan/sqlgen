...
func TestSelect(t *testing.T) {
	assertBuilder(t, NewDefaultBuilder().Select("*").From("dummy").Where("id", "=", 14).OrderBy("created_at DESC"),
		1, "SELECT * FROM dummy WHERE id=$1 ORDER BY created_at DESC")
	assertBuilder(t, NewDefaultBuilder().Select("*").From("user").GroupBy("area").Limit(10).Offset(1),
		0, "SELECT * FROM user GROUP BY area LIMIT 10 OFFSET 1")
	...
	assertBuilder(t, NewBuilder(QuestionArgFunc, "json").Select("*").From("user").Where("id", "=", 1, "AND password", "=", 2),
